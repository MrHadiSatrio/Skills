...
- **Include only what the decision cannot stand without.** The litmus for any content, at every altitude — Context, Decision, and Consequences alike: would the decision still work if the thing described were absent? If yes, it stays out. Campsite work — the hardenings, hygiene sweeps, and latent-defect fixes landed alongside a decision — never enters the record, even when it smooths the decision's mechanics; a defect fix needs no ADR sanction. An accompaniment that is itself ADR-worthy (see "Is this decision ADR-worthy?") earns its own record, never a paragraph in this one.
  - Good: "The drain's process becomes the catalog's sole reader."
  - Bad: "The drain's process becomes the catalog's sole reader; writes now publish atomically via a tmp-file rename, so a reader never lists a payload mid-write." — the relocation works without the atomic publish; the second clause is accompanying hardening, not the decision.
